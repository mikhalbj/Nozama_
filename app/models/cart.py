...
class Cart:
    def __init__(self, account_id, product_id, quantity, total):
...
WHERE cartProduct.account_id = :account_id AND Product.id = CartProduct.product_id
'''
...
FROM CartProduct, Product
WHERE CartProduct.product_id = Product.id AND cartProduct.account_id = :account_id
''',
                              order_id=order_id)
...
@staticmethod
    def get(account_id):
        ...
        return Account(account_id, total)
